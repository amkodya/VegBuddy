...
if ($_SERVER['REQUEST_METHOD'] =='POST') {  // if the request method is a POST (it is)
        ...
        $username = $_POST['username']; //get POST values from form - first name
        $password = $_POST['password']; //get post values from form - password
        $zip = $_POST['zip']; //get post values from form - zip

        $stmt = $dbh->prepare("INSERT INTO users (firstname, lastname, email, username, password, zip) VALUES (:firstname, :lastname, :email, :username, :password, :zip);");  //prepare statement calling clients table and setting var for the different columns
